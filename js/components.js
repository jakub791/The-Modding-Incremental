...
function loadVue() {
  ...
  Vue.component("tree", {
    ...
    template: `<span><div class="blue" :style="{width: tmp.other.screenWidth}"><br><feature></feature><br><br><news></news></div>
  ...
  Vue.component("feature", {
    ...
    computed: {
      current() {
        for (let feature in tmp["info-tab"].features) {
          if (run(tmp["info-tab"].features[feature].unlocked, tmp["info-tab"].features[feature]) === false) return tmp["info-tab"].features[feature]
        }},
      },
    template: `<display-div :data="'<div class=feature>' + current.display + '<br>(' + format(current.progress.mul(100)) + '%)'"></display-div>`
  })
